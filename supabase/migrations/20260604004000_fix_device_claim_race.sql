-- Make device claiming idempotent under fast duplicate auth events from the same browser.
...
BEGIN
  ...
  -- Serialize claims for the same user so two auth events cannot race each other.
